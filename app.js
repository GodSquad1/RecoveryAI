(() => {
    ...
    function calcVocabRichness(text, maxEstimatedG = 0.14) {
        ...
        const denominator = Math.log(maxEstimatedG + 1) ** 2;
  ...
  """${text}"""`;
    ...
    async function generateGptAssessment(entries) {
      const prompt = `You are a stroke recovery specialist. Assess the patient's language progress over 5 sessions:
  ...
  Write a short professional progress note.`;
    ...
    async function addEntry(text) {
      ...
      if (!metrics || typeof metrics.vocab_richness !== 'number') {
    ...
    function updateUI() {
      ...
      grammarFlagDiv.style.color = latest.grammar === 'OK' ? '#4ade80' : '#f87171';
